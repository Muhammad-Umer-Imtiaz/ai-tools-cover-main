...
export default function Page() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        // 🔹 Check if already cached in sessionStorage (using consistent key)
        const cachedPosts = sessionStorage.getItem("posts");

        if (cachedPosts) {
          setBlogs(JSON.parse(cachedPosts));
          setLoading(false);
          return;
        }
  ...
  const filteredBlogs = blogs.filter(
    (blog) =>
      ...
      getExcerpt(blog.content).toLowerCase().includes(searchTerm.toLowerCase())
  );
          ...
          {filteredBlogs.map((blog) => (
            ...
            >
              ...
              <div className="p-6">
                ...
                <p className="text-gray-600 text-sm mb-4 line-clamp-3">{getExcerpt(blog.content)}</p>
                <div className="flex items-center justify-between text-xs text-gray-500 mb-4">
                  <div className="flex items-center gap-3">
                    <span className="flex items-center gap-1">
                      <FiCalendar size={12} /> {formatDate(blog.publishedAt)}
                    </span>
                    <span className="flex items-center gap-1">
